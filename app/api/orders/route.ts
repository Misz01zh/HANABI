...
export async function POST(request: NextRequest) {
  ...
  const { data: movies, error: movieError } = await adminClient.from("movies").select("id, title, price_cents").eq("status", "published").is("deleted_at", null).in("id", uniqueMovieIds).returns<MovieForOrder[]>();
