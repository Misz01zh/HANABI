...
export default function AdminMoviesPage() {
  ...
  return <><style jsx global>{`.admin-fields{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:1rem;margin-bottom:1rem}.admin-fields label{display:grid;gap:.45rem;min-width:0}.admin-fields label>span{color:#b3b3b3;font-size:.9rem;font-weight:600}.admin-fields input,.admin-fields select{width:100%}.admin-field-wide{grid-column:1/-1}@media(max-width:720px){.admin-fields{grid-template-columns:1fr}.admin-field-wide{grid-column:auto}}`}</style><main><h1>影片管理</h1><h2>创建影片</h2><form onSubmit={create}>{fields()}<button>创建影片</button></form><p aria-live="polite">{message}</p><h2>编辑影片</h2>{movies.map(movie => <form key={movie.id} onSubmit={event => update(event, movie.id)}>{fields(movie)}<p>当前状态：{movie.deleted_at ? "已软删除" : movie.status}</p><button>保存修改</button><button type="button" disabled={movie.status === "archived"} onClick={() => void archive(movie.id)}>下架并删除</button></form>)}</main></>;
}
